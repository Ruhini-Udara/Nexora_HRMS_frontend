...
export default function TerminationsExecutionPage() {
    ...
    const [pendingRequests, setPendingRequests] = useState<TerminationRequest[]>([]);
    const [executedRequests, setExecutedRequests] = useState<TerminationRequest[]>([]);
    ...
    const getExecutionDateStr = (req: TerminationRequest) => {
        if (req.updatedAt) return req.updatedAt.split('T')[0];
        if (req.effectiveDate) return req.effectiveDate.split('T')[0];
    ...
    const renderTableRow = (req: TerminationRequest, isPrintMode = false) => (
        <tr key={req.id + (isPrintMode ? "-print" : "")} className="hover:bg-gray-50 dark:hover:bg-slate-800/20 transition-colors print:hover:bg-transparent">
            ...
            <td className="px-6 py-4 print:py-2.5 print:px-3">
                <p className="text-sm text-gray-900 dark:text-white print:text-black">{req.branch}</p>
                <p className="text-xs text-gray-500 print:text-gray-700">{req.type}</p>
            </td>
            {activeTab === "executed" ? (
                <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 print:hidden">
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 ml-10">
                        View approved terminations and execute offboarding updates, or review past executions by date.
                    </p>
                </div>
            ...
            {activeTab === "executed" && (
                <div className="hidden print:block mb-4 border-b border-gray-300 pb-3">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl font-bold text-black uppercase tracking-wide">HR MATE — Termination Executions Report</h1>
                    ...
                    <table className="w-full text-left border-collapse print:w-full">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800 print:bg-gray-100 print:border-b print:border-gray-300">
                                ...
                                <th className="px-6 py-4 print:py-2.5 print:px-3 text-xs font-bold text-gray-500 dark:text-slate-400 print:text-black uppercase tracking-wider">Branch / Type</th>
                                {activeTab === "executed" ? (
                                    <>
                                        ...
                                        {activeTab === "executed" ? (
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p className="font-semibold text-slate-700 dark:text-slate-300">
                                                    No executed terminations found for {showAllDates ? "any date" : (selectedDate ? format(new Date(selectedDate + "T00:00:00"), "MMM dd, yyyy") : "selected date")}.
                                                </p>
                                                {!showAllDates && (
                                                    <button
                                                    ...
                                                    >
                                                        Click here to view all executed terminations across all dates
                                                    </button>
                                                )}
                                            </div>
                                        ) : (
