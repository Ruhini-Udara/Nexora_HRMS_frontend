...
export default function ResignationsExecutionPage() {
    ...
    const [executedRequests, setExecutedRequests] = useState<ResignationRequest[]>([]);
    ...
    const getExecutionDateStr = (req: ResignationRequest) => {
        if (req.updatedAt) return req.updatedAt.split('T')[0];
        if (req.lastWorkingDate) return req.lastWorkingDate.split('T')[0];
        if (req.resignationDate) return req.resignationDate.split('T')[0];
    ...
    const filteredPending = pendingRequests.filter((req) => {
        ...
        const rawDate = req.lastWorkingDate ? req.lastWorkingDate.split('T')[0] : (req.resignationDate ? req.resignationDate.split('T')[0] : todayStr);
    ...
    const renderTableRow = (req: ResignationRequest, isPrintMode = false) => (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 print:hidden">
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 ml-10">
                        View approved resignations and execute offboarding updates, or review past executions by date.
                    </p>
                </div>
            ...
            {activeTab === "executed" && (
                <div className="hidden print:block mb-4 border-b border-gray-300 pb-3">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl font-bold text-black uppercase tracking-wide">HR MATE — Resignation Executions Report</h1>
                    ...
                    <table className="w-full text-left border-collapse print:w-full">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800 print:bg-gray-100 print:border-b print:border-gray-300">
                                ...
                                <th className="px-6 py-4 print:py-2.5 print:px-3 text-xs font-bold text-gray-500 dark:text-slate-400 print:text-black uppercase tracking-wider">Branch</th>
                                {activeTab === "executed" ? (
                                    <>
                                        ...
                                        <th className="px-6 py-4 print:py-2.5 print:px-3 text-xs font-bold text-gray-500 dark:text-slate-400 print:text-black uppercase tracking-wider">Last Working Date</th>
                                    </>
                                ) : (
                                    <th className="px-6 py-4 print:py-2.5 print:px-3 text-xs font-bold text-gray-500 dark:text-slate-400 print:text-black uppercase tracking-wider">Last Working Date</th>
                                )}
                                        ...
                                        {activeTab === "executed" ? (
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p className="font-semibold text-slate-700 dark:text-slate-300">
                                                    No executed resignations found for {showAllDates ? "any date" : (selectedDate ? format(new Date(selectedDate + "T00:00:00"), "MMM dd, yyyy") : "selected date")}.
                                                </p>
                                                {!showAllDates && (
                                                    <button
                                                    ...
                                                    >
                                                        Click here to view all executed resignations across all dates
                                                    </button>
                                                )}
                                            </div>
                                        ) : (
