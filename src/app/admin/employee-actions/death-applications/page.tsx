...
export default function DeathApplicationsExecutionPage() {
    ...
    const [pendingRequests, setPendingRequests] = useState<DeathRequest[]>([]);
    const [executedRequests, setExecutedRequests] = useState<DeathRequest[]>([]);
    ...
    const getExecutionDateStr = (req: DeathRequest) => {
        if (req.updatedAt) return req.updatedAt.split('T')[0];
        if (req.dateOfDeath) return req.dateOfDeath.split('T')[0];
    ...
    // Filter executed requests by calendar date
    ...
    const activeList = activeTab === "executed" ? filteredExecuted : sortedPending;
    const totalPages = Math.ceil(activeList.length / itemsPerPage);
    const paginatedRequests = activeList.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

    const renderTableRow = (req: DeathRequest, isPrintMode = false) => (
        <tr key={req.id + (isPrintMode ? "-print" : "")} className="hover:bg-gray-50 dark:hover:bg-slate-800/20 transition-colors print:hover:bg-transparent">
            ...
            <td className="px-6 py-4 print:py-2.5 print:px-3 text-sm text-gray-600 dark:text-gray-300 print:text-black">
                {req.dateOfDeath ? format(new Date(req.dateOfDeath), "MMM dd, yyyy") : "N/A"}
            </td>
            <td className="px-6 py-4 print:py-2.5 print:px-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white print:text-black">{req.requesterName}</p>
                <p className="text-xs text-gray-500 print:text-gray-700">{req.requesterBranch}</p>
            </td>
            {activeTab === "executed" && (
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto w-full flex-1 print:p-0 print:m-0 print:max-w-none">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 print:hidden">
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 ml-10">
                        View approved death applications and execute offboarding updates, or review past executions by date.
                    </p>
                </div>

                <div className="flex items-center gap-3 self-start sm:self-center">
                    {/* Sort dropdown (for pending list) */}
                    {activeTab === "pending" && (
                        <div className="flex items-center gap-2.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg px-3.5 py-2 shadow-xs">
                    ...
                    {activeTab === "executed" && (
                        <button
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 hover:bg-gray-50 dark:hover:bg-slate-800 text-gray-800 dark:text-gray-200 text-sm font-semibold rounded-xl border border-gray-200 dark:border-slate-800 shadow-xs transition-colors cursor-pointer shrink-0"
            ...
            {activeTab === "executed" && (
                <div className="hidden print:block mb-4 border-b border-gray-300 pb-3">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl font-bold text-black uppercase tracking-wide">HR MATE — Death Applications Execution Report</h1>
            ...
            <div className="flex space-x-1 border-b border-gray-200 dark:border-slate-800 mb-6 overflow-x-auto print:hidden">
                ...
                >
                    Ready for Execution
                    <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 font-semibold">
                    ...
                    <table className="w-full text-left border-collapse print:w-full">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800 print:bg-gray-100 print:border-b print:border-gray-300">
                                ...
                                <th className="px-6 py-4 print:py-2.5 print:px-3 text-xs font-bold text-gray-500 dark:text-slate-400 print:text-black uppercase tracking-wider">Date of Death</th>
                                <th className="px-6 py-4 print:py-2.5 print:px-3 text-xs font-bold text-gray-500 dark:text-slate-400 print:text-black uppercase tracking-wider">Requester</th>
                                {activeTab === "executed" && (
                                        ...
                                        {activeTab === "executed" ? (
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p className="font-semibold text-slate-700 dark:text-slate-300">
                                                    No executed death applications found for {showAllDates ? "any date" : (selectedDate ? format(new Date(selectedDate + "T00:00:00"), "MMM dd, yyyy") : "selected date")}.
                                                </p>
                                                {!showAllDates && (
                                                    <button
                                                    ...
                                                    >
                                                        Click here to view all executed death applications across all dates
                                                    </button>
                                                )}
                                            </div>
                                        ) : (
                        ...
                        {/* Print-only full list table body */}
