...
export default function LeaveApprovalsPage() {
        ...
        return [
            ...
            { 
                ...
                status: req.status === 'PENDING_ADMIN_APPROVAL' ? 'current' as const : 
                        req.status === 'APPROVED' || req.status === 'PENDING_DIRECTOR_REVIEW' ? 'completed' as const : 'pending' as const 
            },
            { 
                label: 'Board Approval', 
                status: req.status === 'PENDING_DIRECTOR_REVIEW' || req.status === 'APPROVED' ? 'current' as const : 'pending' as const 
            }
        ];
    };
    ...
    const handleOpenReview = async (req: OverseasLeave) => {
        ...
        try {
            const res = await api.get(`/api/v1/leaves/overseas/${req.id}/impact`);
