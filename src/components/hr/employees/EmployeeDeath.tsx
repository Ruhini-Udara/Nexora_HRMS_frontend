...
export default function EmployeeDeath() {
    ...
    const loadRequests = useCallback(async () => {
        try {
            ...
            const recentData = data.filter(r => {
                const reqDate = new Date(r.dateOfDeath || '');
    ...
    const handleSaveRequest = async (data: DeathRequest, closeAfterSave = true) => {
        try {
            if (selectedRequest) {
    ...
    const filteredRequests = requests.filter(isLegit).filter(req => {
        ...
        const d = req.createdAt || req.dateOfDeath;
        if (d) {
            ...
            <div className="flex-1 p-8 pb-16 max-w-7xl mx-auto w-full">
                ...
                <div className="mb-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {(
                        ...
                        const statCount = requests.filter(isLegit).filter(r => r.status === status).filter(req => {
                            let matchesTime = true;
                            const d = req.createdAt || req.dateOfDeath;
                            if (d) {
                                const reqDate = new Date(d);
