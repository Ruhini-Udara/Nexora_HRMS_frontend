...
export default function MaternityLeaveApprovals() {
    ...
    const handleApprove = () => {
        ...
        setRequests(prev => prev.map(r => r.id === selectedRequest.id ? { ...r, status: "Approved", adminRemark } : r));
        handleCloseModal();
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-base">
                            Review and manage maternity leave requests requiring final admin approval.
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <tbody className="text-sm">
                            {filteredRequests.map((req) => (
                                <tr key={req.id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${req.status === "Sent for Admin Approval"
                            ...
                            {selectedRequest.status === "Sent for Admin Approval" && (
                                <>
                                    ...
                                    <button onClick={handleApprove} className="px-5 py-2.5 bg-emerald-500 text-white font-bold hover:bg-emerald-600 rounded-lg transition-colors shadow-sm shadow-emerald-500/20 text-sm flex items-center gap-2">
                                        <span className="material-symbols-outlined text-[18px]">check_circle</span> Finalize Approval
                                    </button>
                                </>
                            )}
                        </div>
